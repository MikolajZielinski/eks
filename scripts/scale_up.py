...
def main():
    ...
    input_path = Path(f"outputs/{model_type}/genie/{timestamp}/tetrahedron_soup.ply")
    output_path = Path(f"blender_neuraleditor/{model_type}/{model_type}.ply")
    ...
    # re-read mesh using potentially updated paths and recompute verts
    triangle_mesh = o3d.io.read_triangle_mesh(str(input_path))
    verts = np.asarray(triangle_mesh.vertices)
    verts *= 3.0

    verts -= np.array([1.5, 1.5, 1.5])

    print(f"Verts shape: {verts.shape}")

    triangle_mesh.vertices = o3d.utility.Vector3dVector(verts)
    o3d.io.write_triangle_mesh(str(output_path), triangle_mesh)
